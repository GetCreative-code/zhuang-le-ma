...
export default function InstallExecutor({ installState, onBack, onDone }: {
  ...
  return (
    <div className="space-y-6 slide-up max-w-3xl mx-auto">
      <div className="text-center">
        {running && <h2 className="text-2xl font-bold">Installing...</h2>}
      ...
      <div className="card">
        ...
        <div className="bg-gray-900 text-green-400 rounded-xl p-4 font-mono text-sm max-h-96 overflow-y-auto whitespace-pre-wrap">
          {output || 'Waiting...'}{running && <span className="animate-pulse">|</span>}
        </div>
      </div>
      {!running && <div className="flex gap-3"><button onClick={onBack} className="btn-secondary flex-1">Back</button><button onClick={onDone} className="btn-primary flex-1">Home</button></div>}
    </div>
  );
}
