...
ipcMain.handle('execute-script', async (event, { script, platform }) => {
  return new Promise((resolve) => {
    ...
    try { writeScriptFile(scriptPath, finalScript, platform); }
      ...
      resolve({ success: false, exitCode: -1, output: '', errorOutput: `Write failed: ${err.message}` });
      return;
    }
    ...
    if (platform === 'windows') {
      ...
      proc.on('close', (code) => { try { fs.unlinkSync(scriptPath); } catch (_) {} resolve({ success: code === 0, exitCode: code, output, errorOutput: code !== 0 ? `Exit code: ${code}` : '' }); });
      proc.on('error', (err) => { resolve({ success: false, exitCode: -1, output, errorOutput: `Process error: ${err.message}` }); });
      return;
    }

    let proc;
    if (platform === 'macos') {
      proc = spawn('osascript', ['-e', `do shell script "bash '${scriptPath.replace(/'/g, "'\\''")}'" with administrator privileges`]);
    } else {
